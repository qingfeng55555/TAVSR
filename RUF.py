...
class RU(nn.Module):
    def __init__(self, H, W, in_chans=1, embed_dim_U=32,
        ...
        self.u_transformer_net_3 = U_transformer(H=self.h, W=self.w, dd_in=in_chans * 2, img_size=64, embed_dim=self.embed_dim_u,
                                               win_size=8, token_projection='linear', token_mlp='leff',
                                               modulator=True)

        ### deep feature extraction --> align_feature
        self.u_transformer_net_4 = U_transformer(H=self.h, W=self.w, dd_in=in_chans * 2, img_size=64, embed_dim=self.embed_dim_u,
                                               win_size=8, token_projection='linear', token_mlp='leff',
                                               modulator=True)
        ...
        self.w = W
        self.embed_dim_u = embed_dim_U
        self.u_transformer_net_1 = U_transformer(H=self.h, W=self.w, dd_in=in_chans * 5, img_size=64, embed_dim=self.embed_dim_u,
                                               win_size=8, token_projection='linear', token_mlp='leff',
                                               modulator=True)

        ## U_transformer_net --> align_feature
        self.u_transformer_net_2 = U_transformer(H=self.h, W=self.w, dd_in=in_chans * 5, img_size=64, embed_dim=self.embed_dim_u,
                                               win_size=8, token_projection='linear', token_mlp='leff',
                                               modulator=True)
    ...
    def forward(self, lqs, ref):
        ...
        msk_ref = torch.sigmoid(
            off_msk_ref[:, self.embed_dim_u*2 * 2 * self.deform_ks * self.deform_ks:, ...]  # torch.Size([8, 27, 128, 128])
        )
...
class R3N(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        return out, ref_resnet, lqs_unet
