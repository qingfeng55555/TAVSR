...
def InOutPaddings(x):
    ...
    # 希望输入尺寸是128的整数倍，因为这样前面正好downshuffle. 这里应该修改成LR的基准。如果patchsize是240，LR就是60.
    if w != ((w >> 7) << 7):
...
class ResBlock(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size=3, reduction=False, bias=True,
        ...
        self.body = nn.Sequential(
            nn.Conv2d(in_feat, out_feat, kernel_size=kernel_size, stride=1, padding=1),
            act,
            nn.Conv2d(out_feat, out_feat, kernel_size=kernel_size, stride=1, padding=1)
        )
...
class CALayer(nn.Module):
    ...
    def forward(self, x):
        #x = torch.Size([16, 64, 64, 64])
        y = self.avg_pool(x) # y = torch.Size([16, 64, 1, 1])
        # B C 1 1
        y = self.conv_du(y) # y = torch.Size([16, 64, 1, 1])
...
class fusion_model(nn.Module):
    def __init__(self, channel, reduction=8):
        ...
        self.conv_1 = nn.Conv2d(channel*2, channel, 3, stride=1, padding=1, bias=True)
        self.relu = nn.ReLU(inplace=True)
        self.conv_2 = nn.Conv2d(channel, channel, 3, stride=1, padding=1, bias=True)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
class RCAB(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size, reduction, bias=True,
        ...
        self.body = nn.Sequential(
            nn.Conv2d(in_feat, out_feat, kernel_size, stride=1, padding=1),
            act,
            nn.Conv2d(out_feat, out_feat, kernel_size, stride=1, padding=1),
    ...
    def forward(self, x):
        res = x
        out, ca = self.body(x) # out = torch.Size([16, 64, 64, 64])

        if self.downscale:
            res = self.downConv(res)
        out += res # out = torch.Size([16, 64, 64, 64])
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x):
        # x = torch.Size([16, 64, 64, 64])
        res = self.body(x) # x = torch.Size([16, 64, 64, 64])
...
def pixel_shuffle(input, scale_factor):
    # input = torch.Size([8, 256, 64, 64])
    batch_size, channels, in_height, in_width = input.size()
    # batch_size, in_height, in_width, channels = input.size()
...
# RIR :     
class RIR(nn.Module):
    def __init__(self, n_resgroups, n_resblocks, n_feats, concat_feats,
...
concat_feats: 64
    ...
    def forward(self, x):
        ...
        # x = torch.Size([16, 64, 64, 64])
        res = self.body(x) # res = torch.Size([16, 64, 64, 64])

        res += x

        out = self.tailConv(res) # out = torch.Size([16, 64, 64, 64])
...
class Refsr(nn.Module):
    def __init__(self, n_resgroups, n_resblocks, n_feats, in_channel,
                 reduction=16, act=nn.LeakyReLU(0.2, True), norm=False):
        super(Refsr, self).__init__()

        # define modules: head, body, tail
        scale = 4
        concat_feats = in_channel + 3*(scale**2)
        self.headConv = conv3x3(concat_feats, n_feats)
...
class Sisr(nn.Module):
    def __init__(self, n_resgroups, n_resblocks, n_feats,
        ...
        # define modules: head, body, tail
        in_channel = 3
        self.headConv = conv3x3(3, n_feats)
...
# Interpolation : 2frames → downshuffle → 2*n_feats → n_feats → upshuffle → Interframe
...
class resnet_transformer(nn.Module):
    def __init__(self, img_size=64, patch_size=48, in_chans=3,
                 embed_dim=180, depths=[6, 6, 6, 6, 6, 6], num_heads=[6, 6, 6, 6, 6, 6],
                 window_size=32, mlp_ratio=2, qkv_bias=True, qk_scale=None,
    ...
    def forward(self, fus):
        ...
        ### deep transformer net
        x_size = (fus.shape[2], fus.shape[3])
        y = self.patch_embed(fus)
        # if self.ape:
