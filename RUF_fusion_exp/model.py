...
class FastLeFF(nn.Module):
    ...
    def __init__(self, h, w, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0.):
        super().__init__()
...
class UNet(nn.Module):
    def __init__(self, block=ConvBlock, dim=32, in_channel=3):
        ...
        self.ConvBlock5 = block(dim * 4, dim * 8, strides=1)
        ...
        self.conv10 = nn.Conv2d(dim, 2*dim, kernel_size=3, stride=1, padding=1)
...
class LeFF(nn.Module):
    def __init__(self, h, w, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0., use_eca=False):
        super().__init__()
        self.linear1 = nn.Sequential(nn.Linear(dim, hidden_dim),
                                     act_layer())
...
class OutputProj(nn.Module):
    def __init__(self, h, w, in_channel=64, out_channel=3, kernel_size=3, stride=1, norm_layer=None, act_layer=None):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=kernel_size // 2),
        )
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, h, w, win_size=8, shift_size=0,
        ...
        if token_mlp in ['ffn', 'mlp']:
            ...
            self.mlp = LeFF(h=self.h, w=self.w ,dim=dim ,hidden_dim=mlp_hidden_dim, act_layer=act_layer, drop=drop)

        elif token_mlp == 'fastleff':
            self.mlp = FastLeFF(h=self.h, w=self.w ,dim=dim ,hidden_dim=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        else:
    ...
    def forward(self, x, mask=None):
        ...
        if mask != None:
            ...
            h, w, input_mask_windows = window_partition(input_mask, self.win_size)  # nW, win_size, win_size, 1
        ...
        if self.shift_size > 0:
            ...
            for h in h_slices:
                for w in w_slices:
                    shift_mask[:, h, w, :] = cnt
                    cnt += 1
            h, w, shift_mask_windows = window_partition(shift_mask, self.win_size)  # nW, win_size, win_size, 1
        ...
        # partition windows
        h, w, x_windows = window_partition(shifted_x, self.win_size)  # nW*B, win_size, win_size, C  N*C->C
        ...
        shifted_x = window_reverse(attn_windows, self.win_size, H=h, W=w)  # B H' W' C
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size, h, w,
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                      ...
                                      modulator=modulator, cross_modulator=cross_modulator, h=self.h, w=self.w)
...
class U_transformer(nn.Module):
    def __init__(self, H, W, img_size=64,dd_in=3,
                 embed_dim=64, depths=[2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 8, 4, 2],
        ...
        self.output_proj = OutputProj(h=H, w=W, in_channel=2 * embed_dim, out_channel=embed_dim*2, kernel_size=3, stride=1)
        ...
        self.encoderlayer_0 = BasicUformerLayer(dim=embed_dim,
                                                ...
                                                shift_flag=shift_flag, h=H, w=W)
        self.dowsample_0 = dowsample(embed_dim, embed_dim * 2, h=H, w=W)
        self.encoderlayer_1 = BasicUformerLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(H//2,
                                                                  W//2),
                                                depth=depths[1],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                shift_flag=shift_flag, h=H//2, w=W//2)
        self.dowsample_1 = dowsample(embed_dim * 2, embed_dim * 4, h=H//2, w=W//2)
        self.encoderlayer_2 = BasicUformerLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(H//(2**2),
                                                                  W//(2**2)),
                                                depth=depths[2],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                shift_flag=shift_flag, h=H//(2**2), w=W//(2**2))
        self.dowsample_2 = dowsample(embed_dim * 4, embed_dim * 8, h=H//(2**2), w=W//(2**2))
        ...
        self.upsample_0 = upsample(embed_dim * 8, embed_dim * 4, h=H//(2**3), w=W//(2**3))
        self.decoderlayer_0 = BasicUformerLayer(dim=embed_dim * 8,
                                                ...
                                                modulator=modulator, cross_modulator=cross_modulator
                                                , h=H//(2**2), w=W//(2**2))
        self.upsample_1 = upsample(embed_dim * 8, embed_dim * 2, h=H//(2**2), w=W//(2**2))
        self.decoderlayer_1 = BasicUformerLayer(dim=embed_dim * 4,
                                                ...
                                                modulator=modulator, cross_modulator=cross_modulator
                                                , h=H//2, w=W//2)
        self.upsample_2 = upsample(embed_dim * 4, embed_dim, h=H//2, w=W//2)
...
#     net = U_transformer(H=64,W=64).cuda()
#     from thop import profile

#     input = torch.randn(2, 15, 64, 64).cuda()
#     out = net(input)
    # print(out)
