...
def main(video_num, video_name, H, W, Image_LQlist, Image_HQlist, Image_Reflist, ckp_path, nfs):
    ...
    for idx in range(nfs):
        ...
        with torch.no_grad():
            enhanced_frm, up_frm = split_enhance(model, input_data, ref_data)
...
def split_enhance(model, inputs, ref_data):
    outputs = ref_data.clone().detach()
    up_frm  = ref_data.clone().detach()
    if inputs.shape[4] == 1280: # 分成2份
        h_parts = 2
        w_parts = 2
    elif inputs.shape[4] == 960:   # 分成4份
        h_parts = 1
        w_parts = 2
    # elif inputs.shape[4] == 1280: # 分成8份
    #     h_parts = 2
    #     w_parts = 4
    else: # 不分
        h_parts = 1
    ...
    hh = inputs.shape[3] // h_parts
    ww = inputs.shape[4] // w_parts
    hh_ref = ref_data.shape[2] // h_parts
    ww_ref = ref_data.shape[3] // w_parts
    for i in range(h_parts):
        for j in range(w_parts):
            split_inputs = inputs[:, :, :, i * hh:(i + 1) * hh, j * ww:(j + 1) * ww].clone().detach()
            split_ref = ref_data[:, :, i * hh_ref:(i + 1) * hh_ref, j * ww_ref:(j + 1) * ww_ref].clone().detach()

            # print(f"{i * hh}:{(i + 1) * hh},{j * ww}:{(j + 1) * ww} ")
            # print("split shape", split_inputs.shape)
            # print("split shape", split_ref.shape)
            outputs[:, :, i * hh_ref:(i + 1) * hh_ref, j * ww_ref:(j + 1) * ww_ref], up_frm[:, :, i * hh_ref:(i + 1) * hh_ref, j * ww_ref:(j + 1) * ww_ref], _, _ = model(split_inputs, split_ref)
    return outputs, up_frm
...
def val(ckp_num_pth):
    ...
    #                         '021', '022'])
    #
    # video_name = np.array(['ParkScene_1920x1080_240','RaceHorses_416x240_300','RaceHorses_832x480_300','Traffic_2560x1600_150'])

    # video_num = np.array(['001','005','016', '018'])
    
    video_name = np.array(['ParkScene_1920x1080_240', 'BQMall_832x480_600', 'Kimono_1920x1080_240', 'KristenAndSara_1280x720_600', 'RaceHorses_416x240_300',
                    'PartyScene_832x480_500', 'Cactus_1920x1080_500', 'BlowingBubbles_416x240_500', 'FourPeople_1280x720_600', 'Johnny_1280x720_600',
                    'PeopleOnStreet_2560x1600_150', 'BQSquare_416x240_600', 'RaceHorses_832x480_300', 'BasketballDrive_1920x1080_500', 'Traffic_2560x1600_150', 'BasketballDrill_832x480_500',
                    'BasketballPass_416x240_500', 'BQTerrace_1920x1080_600'])
    
    video_num = np.array(['001', '002', '003', '004', '005', 
                            '008', '010', '011', '012', '013',
    ...
    for i in range(0, len(video_num)):
        raw_yuv_path = '/data1/zz/data_wl/RUF_data/test_25_raw_pic/' + video_num[i]
        lq_yuv_path = '/data1/zz/data_wl/RUF_data/test_lqs/bicubic_test_qp47_data_pic/' + video_num[i]
        ref_yuv_path = '/data1/zz/data_wl/RUF_data/raw_ref_pic/test_ref_qp51/' + video_num[i]
...
if __name__ == '__main__':
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_50000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_60000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_70000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_80000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_90000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_100000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_110000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_120000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_130000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_140000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_150000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_160000.pth'))
    print(val(ckp_num_pth='/data1/zz/data_wl/exp_RUF_infinity/300_RUF_qp47_refqp51/ckp_170000.pth'))
# aa = input_data[0,0,:,:,:]
# SR_img = transforms.ToPILImage()(aa.cpu())
# SR_img.save('xxx.png')
